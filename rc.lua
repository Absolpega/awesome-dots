...
function new_rect(cr, width, height)
	...
	s.mytaglist = awful.widget.taglist {
		...
		widget_template = {
			...
			create_callback = function(self, c3, index, objects) --luacheck: no unused args
				local margin = self:get_children_by_id'text_background'[1]
				local text = self:get_children_by_id'text_role'[1]

				self:connect_signal('mouse::enter', function()
					if margin.bg ~= beautiful.tagbox_hover_bg then
						margin.backup_bg = margin.bg
						margin.backup_fg = margin.fg

						margin.has_backup = true
					end
					margin.bg = beautiful.tagbox_hover_bg
					margin.fg = beautiful.tagbox_hover_fg

					text.fg = margin.fg
				end)
				self:connect_signal('mouse::leave', function()
					if margin.has_backup then
						margin.bg = margin.backup_bg
						margin.fg = margin.backup_fg
					end
					text.fg = margin.fg
				end)
			end,
